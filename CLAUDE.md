...
1. `GET http://localhost:3000/api/evaluate?index={순번}` 또는 `?team={팀이름}`으로 프로젝트 데이터 조회
2. 응답의 `evaluationPrompt`(평가 프롬프트)를 기반으로 심사 수행
3. 프로젝트의 GitHub/데모/영상 URL이 있으면 접근하여 실제 내용 확인
4. 평가 결과를 `POST http://localhost:3000/api/evaluate`로 DB 저장 (`projectId` + `promptFeedback`)
5. 디스코드에 심사 결과 회신

### 심사 결과 조회

`@삿 심사결과 {순번}` 또는 `@삿 심사결과 {팀이름}`

1. `GET http://localhost:3000/api/evaluate?index={순번}` 또는 `?team={팀이름}`으로 조회
2. 응답의 `promptFeedback` 값을 디스코드에 회신 (없으면 "아직 심사 결과 없음" 안내)
...
1. `GET http://localhost:3000/api/evaluate?index={순번}` 또는 `?team={팀이름}`으로 프로젝트 데이터 조회
2. 프로젝트의 `githubUrl`이 공개 레포인지 확인
3. `gh api`로 레포 구조, 주요 파일, 코드 내용에 직접 접근
4. 기술 스택, 프로젝트 구조, 코드 품질, 주요 기능 등을 분석
5. 분석 결과를 디스코드에 회신
